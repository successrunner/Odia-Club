...
import ProposalTab from '@/containers/ProposalTab';
import { useGetBranding } from '@/hooks/useGetBranding';
import { useGetProposalSummary } from '@/hooks/useGetProposalSummary';

export default function ProposalPage() {
  const params = useLocalSearchParams<{ slug: string[]; draft?: string }>();

  // For URL /p/my-post -> slug = ['my-post']
  // For URL /p/my-post/v/1 -> slug = ['my-post', 'v', '1']
  const slug = Array.isArray(params.slug) ? params.slug : [params.slug];

  const shareCode = slug[0];
  ...
  useEffect(() => {
    getSummary({ shareCode, version, draft: isDraft });
    getBranding(shareCode);
  }, [shareCode, version, isDraft, getBranding, getSummary]);
      ...
      >
        ...
        {activeTab === 'itinerary' && (
          <View className="mt-10 w-full max-sm:mt-5">
            <ProposalTab theme={branding.theme} summary={summary} />
          </View>
        )}
        {activeTab === 'pricing' && (
